import { APP_NAME, APP_TAGLINE, BRAND } from '../lib/appMeta';
import { THEME, BRAND as THEME_BRAND } from '../lib/theme';
...
import { insertIncome } from './incomeApi';
import SupabaseExpensesLoader from './SupabaseExpensesLoader';
import { insertExpense } from './expensesApi';
import SupabaseBrandDealsLoader from './SupabaseBrandDealsLoader';
import { insertBrandDeal, updateBrandDealWithIncomeSync, deleteBrandDeal as deleteBrandDealApi } from './brandDealsApi';
...
export default function CreatorFlow() {
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('cf_active_tab') || 'dashboard';
    }
    return 'dashboard';
  });
  useEffect(() => {
    try { localStorage.setItem('cf_active_tab', activeTab); } catch {}
  }, [activeTab]);
  ...
  const handleAddIncome = async () => {
    try {
      ...
      const row = await insertIncome(newIncome);
      // normalize & reflect in UI immediately
      ...
      console.log('[IncomeUI] Inserted & updated list.');
    } catch (e) {
      console.error('[IncomeUI] Insert failed:', e);
  ...
  const handleAddExpense = async () => {
    try {
      if (!newExpense.description || !newExpense.amount) {
        alert('Please enter a description and amount'); return;
      }
      const row = await insertExpense(newExpense);
      setExpenses(prev => [
        ...prev,
        {
          id: row.id,
      ...
      console.log('[ExpensesUI] Inserted & updated list.');
    } catch (e) {
      console.error('[ExpensesUI] Insert failed:', e);
      alert('Insert failed: ' + (e?.message || e));
    }
  };
  ...
  const handleAddBrandDeal = async () => {
    try {
      if (!newBrandDeal.brand || !newBrandDeal.campaign || !newBrandDeal.amount) {
        alert('Please fill Brand, Campaign, and Amount'); return;
      }
      const row = await insertBrandDeal(newBrandDeal);
      setBrandDeals(prev => [...prev, {
        id: row.id,
        brand: row.brand || '',
      ...
      console.log('[BrandDealsUI] Inserted & updated list.');
    } catch (e) {
      console.error('[BrandDealsUI] Insert failed:', e);
      alert('Insert failed: ' + (e?.message || e));
    }
  };
  ...
  // ===== Calendar helpers =====
  const handleCalendarDayClick = (dayOrDateStr) => {
    let dateStr = '';
    if (typeof dayOrDateStr === 'string' && dayOrDateStr.includes('-')) {
      ...
      const d = String(dayOrDateStr).padStart(2,'0');
      dateStr = `${y}-${m}-${d}`;
    }
    ...
    try {
      ...
      if (field === 'amount') {
        if (value === '' || value === null || value === undefined) {
          alert('Amount cannot be empty'); return;
        }
        const n = parseFloat(value);
        if (Number.isNaN(n)) { alert('Amount must be a number'); return; }
        newVal = n;
      }

      const updates = { [field]: newVal || null };
      ...
      if (error) {
        console.error('[IncomeDB] update error:', error);
        alert('Update failed: ' + error.message);
        return;
      }

      setIncomeEntries(prev => prev.map(e => e.id === id ? {
        ...e,
        id: data.id,
        ...
        amount: typeof data.amount === 'number' ? data.amount : parseFloat(data.amount || e.amount || '0'),
        category: data.category ?? e.category,
      } : e));
      setEditingIncome(null);
      console.log('[IncomeDB] updated:', id, field, newVal);
    } catch (err) {
      console.error('[IncomeDB] update failed:', err);
  ...
  const deleteIncome = async (id) => {
    try {
      ...
      if (error) {
        console.error('[IncomeDB] delete error:', error);
        alert('Delete failed: ' + error.message);
        return;
      }

      setIncomeEntries(prev => prev.filter(e => e.id !== id));
      console.log('[IncomeDB] deleted:', id);
    } catch (e) {
      console.error('[IncomeDB] delete failed:', e);
  ...
  const deleteExpense = async (id) => {
    try {
      ...
      if (error) { console.error('[ExpensesDB] delete error:', error); alert('Delete failed: ' + error.message); return; }

      setExpenses(prev => prev.filter(e => e.id !== id));
      console.log('[ExpensesDB] deleted:', id);
    } catch (e) {
      console.error('[ExpensesDB] delete failed:', e);
      alert('Delete failed: ' + (e?.message || e));
    }
  };
  ...
  const updateBrandDeal = async (id, field, value) => {
    try {
      const row = await updateBrandDealWithIncomeSync(id, field, value);
      setBrandDeals(prev => prev.map(d => d.id === id ? ({
        id: row.id,
        brand: row.brand || '',
        campaign: row.campaign || '',
        stage: row.stage || 'Outreach',
      ...
      setEditingBrandDeal(null);
      console.log('[BrandDealsDB] updated:', id, field, value);
    } catch (e) {
      console.error('[BrandDealsDB] update failed:', e);
      alert('Update failed: ' + (e?.message || e));
    }
  };

  const deleteBrandDeal = async (id) => {
    try {
      await deleteBrandDeal(id);
      setBrandDeals(prev => prev.filter(d => d.id !== id));
      console.log('[BrandDealsDB] deleted:', id);
    } catch (e) {
      console.error('[BrandDealsDB] delete failed:', e);
      alert('Delete failed: ' + (e?.message || e));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50">
      <SupabaseIncomeLoader onLoad={setIncomeEntries} />
      <SupabaseExpensesLoader onLoad={setExpenses} />
      <SupabaseBrandDealsLoader onLoad={setBrandDeals} />
      {/* Calendar Day Modal */}
      {showCalendarModal && (
        <CalendarModal
          date={selectedCalendarDay}
          incomeItems={(incomeEntries || []).filter(e => e.date === selectedCalendarDay)}
          expenseItems={(expenses || []).filter(e => e.date === selectedCalendarDay)}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-900 to-cyan-700 bg-clip-text text-transparent tracking-tight">{APP_NAME}</h1>
                  ...
                  <div className="bg-white rounded-xl shadow p-6">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span>${monthlyNet.toLocaleString()} of ${monthlyGoal.toLocaleString()} this month ({goalProgress}%)</span>
                              <input type="number" min="0" value={monthlyGoal} onChange={(e) => setMonthlyGoal(parseFloat(e.target.value) || 0)} className="w-24 text-sm border border-slate-300 rounded-lg px-2 py-1 focus:border-indigo-500 focus:ring-1 focus:ring-purple-500" />
                      {goalSurplus > 0 && (
                    ...
                    <h4 className="font-medium text-slate-800 mb-4">Add New Income</h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input
                          ...
                          className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1 focus:ring-purple-500"
                        />
                        <input
                          type="text"
                          placeholder="e.g., TikTok Shop Commission, Brand Deal Name"
                      ...
                      <input type="text" placeholder="Platform (Instagram, TikTok, etc.)" value={newIncome.platform} onChange={(e) => setNewIncome({ ...newIncome, platform: e.target.value })} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <input type="text" placeholder="Description" value={newIncome.description} onChange={(e) => setNewIncome({ ...newIncome, description: e.target.value })} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <div className="flex gap-2">
                        <input type="number" placeholder="Amount ($)" value={newIncome.amount} onChange={(e) => setNewIncome({ ...newIncome, amount: e.target.value })} className="border border-slate-300 rounded-lg px-3 py-2 flex-1 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                        <button onClick={handleAddIncome} className="bg-gradient-to-r from-indigo-600 to-cyan-600 text-white px-4 py-2 rounded-lg hover:from-indigo-700 hover:to-cyan-700 transition-all duration-200">
                    ...
                    {incomeEntries.map(entry => (
                      <div key={entry.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-cyan-50 to-indigo-50 rounded-lg border border-cyan-100">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => setEditingIncome(editingIncome === entry.id ? null : entry.id)} className=\1text-purple-700\4hover:text-indigo-800 transition-colors">
                ...
                <div>
                  ...
                  <div className="bg-slate-50 p-6 rounded-xl mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <input type="date" value={newExpense.date} onChange={(e) => setNewExpense({...newExpense, date: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <select value={newExpense.category} onChange={(e) => setNewExpense({...newExpense, category: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4>
                      ...
                      <input type="text" placeholder="Description" value={newExpense.description} onChange={(e) => setNewExpense({...newExpense, description: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <div className="flex gap-2">
                        <input type="number" placeholder="Amount ($)" value={newExpense.amount} onChange={(e) => setNewExpense({...newExpense, amount: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 flex-1 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => setShowTaxInfo(true)} className=\1text-purple-700\4hover:text-indigo-800 transition-colors">
                            <Info className="h-4 w-4" />
                          </button>
                        </div>
                        <button onClick={handleAddExpense} className="bg-gradient-to-r from-indigo-600 to-cyan-600 text-white px-4 py-2 rounded-lg hover:from-indigo-700 hover:to-cyan-700 transition-all duration-200">
                          <PlusCircle className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
              ...
              {activeTab === 'brands' && (
                <div>
                  ...
                  <div className="bg-slate-50 p-6 rounded-xl mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <input type="text" placeholder="Brand name" value={newBrandDeal.brand} onChange={(e) => setNewBrandDeal({...newBrandDeal, brand: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <input type="text" placeholder="Campaign name" value={newBrandDeal.campaign} onChange={(e) => setNewBrandDeal({...newBrandDeal, campaign: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <select value={newBrandDeal.stage} onChange={(e) => setNewBrandDeal({...newBrandDeal, stage: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4>
                      ...
                      <select value={newBrandDeal.paymentStatus} onChange={(e) => setNewBrandDeal({...newBrandDeal, paymentStatus: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4>
                        <option value="Pending">Pending</option>
                        <option value="Not Due">Not Due</option>
                        <option value="Overdue">Overdue</option>
                        <option value="Paid">Paid</option>
                      </select>
                      <input type="date" value={newBrandDeal.dueDate} onChange={(e) => setNewBrandDeal({...newBrandDeal, dueDate: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <input type="number" placeholder="Deal value ($)" value={newBrandDeal.amount} onChange={(e) => setNewBrandDeal({...newBrandDeal, amount: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <input type="text" placeholder="Notes" value={newBrandDeal.notes} onChange={(e) => setNewBrandDeal({...newBrandDeal, notes: e.target.value})} className="border border-slate-300 rounded-lg px-3 py-2 md:col-span-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4 />
                      <button onClick={handleAddBrandDeal} className="bg-gradient-to-r from-indigo-600 to-cyan-600 text-white px-4 py-2 rounded-lg hover:from-indigo-700 hover:to-cyan-700 transition-all duration-200 flex items-center gap-2 md:col-span-2">
                        <PlusCircle className="h-4 w-4" />
                        Add Brand Deal
                      </button>
                    </div>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm bg-gradient-to-r from-indigo-100 to-indigo-200\1text-purple-700\4px-3 py-1 rounded-full">{deal.campaign}</span>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button onClick={() => setEditingBrandDeal(editingBrandDeal === deal.id ? null : deal.id)} className=\1text-purple-700\4hover:text-indigo-800 transition-colors">
                                <Edit3 className="h-4 w-4" />
                              </button>
              ...
              {activeTab === 'tax' && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className=\1bg-purple-600\4border border-indigo-200 p-6 rounded-xl">
                      <h4 className="font-medium\1text-purple-700\4mb-4">Tax Calculator</h4>
                      <div className="space-y-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1\1focus:ring-purple-500\4
                            min="0"
                            max="50"
                          />
                        </div>
