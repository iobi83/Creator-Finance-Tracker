...
export default async function handler(req, res) {
  ...
  const updByCustomer = async (c, b) => {
    try {
      const h = {'apikey': _key, 'Authorization': `Bearer ${_key}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal'};
      if (!c || !_key || !_url) { console.log('[updByCustomer:skip]', { hasC: !!c }); return; }
      const r = await fetch(_url + `?stripe_customer_id=eq.${encodeURIComponent(c)}`, { method: 'PATCH', headers: h, body: JSON.stringify(b) });
      console.log('[updByCustomer:res]', { status: r.status });
    } catch (e) { console.log('[updByCustomer:err]', e.message); }
  };
  ...
  if(evt.type==='invoice.payment_succeeded'){const paid=Number((o&&o.amount_paid)||0);if(paid>0){if(!_uid&&_email){await _ent({email:_email,plan:'premium_monthly',stripe_customer_id:o.customer||null});}}else{console.log('[ent:invoice:skip]',{amount_paid:o?.amount_paid});}}
